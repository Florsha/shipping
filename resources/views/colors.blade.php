@extends('layouts.app')

@section('title', 'Table Page')

@section('table')
<div class="table-responsive">
    <h2 class="text-center mb-4">Another Table</h2>
    <table class="table table-hover">
        <thead>
            <tr>
                <th colspan="9" class="text-center">Incoming</th>
                <th colspan="9" class="text-center">Outgoing</th>
            </tr>
            <tr>
                <th scope="col" class="text-center">Container #</th>
                <th scope="col" class="text-center">Size</th>
                <th scope="col" class="text-center">Description</th>
                <th scope="col" class="text-center">OPCODE</th>
                <th scope="col" class="text-center">Vessel</th>
                <th scope="col" class="text-center">Voyage</th>
                <th scope="col" class="text-center">Date and Time</th>
                <th scope="col" class="text-center">Expire Date</th>
                <th scope="col" class="text-center">Remaining Days</th>
                <!-- Outgoing columns -->
                <th scope="col" class="text-center">OPCODE</th>
                <th scope="col" class="text-center">Vessel</th>
                <th scope="col" class="text-center">Voyage</th>
                <th scope="col" class="text-center">Date and Time</th>
                <th scope="col" class="text-center">Expire Date</th>
                <th scope="col" class="text-center">Remaining Days</th>
                <th scope="col" class="text-center">Remarks</th>
            </tr>
        </thead>
        <tbody>
            <tr>
                <th scope="row">1</th>
                <td>AESU-960359-2</td>
                <td>SJDAJDA</td>
                <td>@ADSALDKA</td>
                <td>Mark</td>
                <td>Otto</td>
                <td>@mdo</td>
                <td>Mark</td>
                <td>Otto</td>
                <!-- Outgoing columns -->
                <td>@mdo</td>
                <td>Mark</td>
                <td>Otto</td>
                <td>@mdo</td>
                <td>Mark</td>
                <td>Otto</td> 
                <td>hi</td>
            </tr>
            <tr>
                <th scope="row">2</th>
                <td>SEGU-968594-9</td>
                <td>Otto</td>
                <td>@mdo</td>
                <td>Mark</td>
                <td>Otto</td>
                <td>@mdo</td>
                <td>Mark</td>
                <td>Otto</td>
                <!-- Outgoing columns -->
                <td>@mdo</td>
                <td>Mark</td>
                <td>Otto</td>
                <td>@mdo</td>
                <td>Mark</td>
                <td>Otto</td>
                <td>hi</td>
            </tr>
        </tbody>
    </table>
</div>
@endsection